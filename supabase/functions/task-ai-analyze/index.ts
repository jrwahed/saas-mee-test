...
رد بالعامية المصرية المحترمة. 8 سطور بالظبط — كل سطر فيه معلومة مفيدة.`;
      ...
      case "split_brief":
        systemPrompt = `أنت مدير مشاريع محترف. لما حد يديك brief أو وصف مشروع، قسّمه لمهام عملية واضحة. كل مهمة ليها عنوان، وصف، أولوية، تصنيف، وساعات مقدرة. كلامك بالعربي المصري.

رد بـ JSON بس — بدون أي كلام تاني:
{
  "tasks": [
    {
      "title": "عنوان المهمة",
      "description": "وصف تفصيلي",
      "category": "marketing|design|content|technical|sales|admin",
...
القواعد:
- قسّم البريف لمهام عملية واضحة (3-10 مهام)
- رتّبهم بالأولوية والترتيب المنطقي
- كل مهمة لازم تكون واضحة ومحددة — مش عامة
- الساعات المقدرة لازم تكون واقعية
- التصنيف لازم يكون واحد من: marketing, design, content, technical, sales, admin`;
        userPrompt = `قسّم البريف ده لمهام:
العنوان: ${data.title}
الوصف: ${data.description}
الـ Deadline النهائي: ${data.deadline || 'غير محدد'}
ملاحظات إضافية: ${data.notes || 'مفيش'}`;
        break;
    ...
    // For split_brief, extract JSON from markdown code blocks if present
    if (type === "split_brief") {
